...
def mock_elevenlabs_sdk_client():
    ...
    # Patch where ElevenLabs is looked up by the code under test (elevenlabs_sfx.client)
    with patch('elevenlabs_sfx.client.ElevenLabs', autospec=True) as mock_sdk_constructor:
...
def test_client_initialization_success():
    ...
    with patch('elevenlabs_sfx.client.ElevenLabs') as mock_constructor: # Patched where it's looked up
